...
$PAGE->set_url(new moodle_url('/local/adminmenu/index_en.php'));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Simpan Menu Name
    set_config('adminmenu_name_en', $menu_name, 'local_adminmenu');
    ...
    for ($i = 1; $i <= 10; $i++) {
        ...
        set_config("submenu_en_$i", $submenu, 'local_adminmenu');
        set_config("menu_url_en_$i", $menu_url, 'local_adminmenu');
    }
...
$stored_menu_name = get_config('local_adminmenu', 'adminmenu_name_en');

// Ambil submenu dan URL yang tersimpan
$submenus = [];
$menu_urls = [];
for ($i = 1; $i <= 10; $i++) {
    $submenus[$i] = get_config('local_adminmenu', "submenu_en_$i") ?: '';
    $menu_urls[$i] = get_config('local_adminmenu', "menu_url_en_$i") ?: '';
}
